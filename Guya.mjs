...
var parcelRequire = $parcel$global["parcelRequire19a8"];
if (parcelRequire == null) {
  ...
  $parcel$global["parcelRequire19a8"] = parcelRequire;
}
...
class $72e1f5f1f17e0ef6$export$65d2edca8b0eeeaf extends $8e5b7fb3ed8b7bd1$export$eeddbf09bb970356 {
    async getPopularManga(page4) {
        return this.searchManga(page4);
    }
    async getLatestManga(page1) {
        let body = await $8e5b7fb3ed8b7bd1$export$e7aa7bc5c1b3cfb3(`${this.url}/api/get_all_series`).then((res)=>res.json()
        );
        let data = new Map(Object.keys(body).sort((a, b)=>{
            if (body[a].last_updated > body[b].last_updated) return 1;
            if (body[a].last_updated < body[b].last_updated) return -1;
            return 0;
        }).map((title)=>[
                title,
        ...
        let manga = this.mapResToManga(page1, data);
    ...
    mapResToManga(page2, data, query) {
        let manga = Array.from(data).map(([title, detail])=>({
                sourceId: this.id,
                title: title,
                author: [
                    detail.author,
        ...
        if (query) manga = manga.filter((m)=>m.title.search(query)
        );
        if (page2 < 1) page2 = 1;
        let offset = (page2 - 1) * 20;
    ...
    async searchManga(page3, query1, filter) {
        ...
        let data = new Map(Object.keys(body).sort(this.sortByTitle).map((title)=>[
                title,
                body[title]
            ]
        ));
        let manga = this.mapResToManga(page3, data, query1);
        return Promise.resolve(manga);
    }
    async getMangaDetail(path) {
        let body = await $8e5b7fb3ed8b7bd1$export$e7aa7bc5c1b3cfb3(`${this.url}${path}`).then((res)=>res.json()
        );
    ...
    async getChapters(path1) {
        let body = await $8e5b7fb3ed8b7bd1$export$e7aa7bc5c1b3cfb3(`${this.url}${path1}`).then((res)=>res.json()
        );
        let chapters = Object.keys(body.chapters).map((number)=>{
            ...
            return {
                ...
                uploaded: $fZsJw(ch.release_date[group], $fZsJw.ISO_8601).unix()
            };
        });
        return Promise.resolve(chapters);
    }
    async getPages(path2) {
        let number = path2.substring(path2.lastIndexOf('/') + 1, path2.length);
        path2 = path2.substring(0, path2.lastIndexOf('/'));
        let body = await $8e5b7fb3ed8b7bd1$export$e7aa7bc5c1b3cfb3(`${this.url}${path2}`).then((res)=>res.json()
        );
        let ch = body.chapters[number];
        let group = Object.keys(ch.groups)[0];
...
class $16d48e94867a0a9d$export$2e2bcd8739ae039 extends $72e1f5f1f17e0ef6$export$65d2edca8b0eeeaf {
    constructor(...args){
        super(...args);
        this.id = 7;
        this.name = "Guya";
        this.url = "https://guya.moe";
        this.version = "0.1.1";
        this.icon = "https://guya.moe/static/logo_small.png";
        this.languages = "end";
        this.nsfw = false;
    }
}
...
export {$16d48e94867a0a9d$export$2e2bcd8739ae039 as default};
